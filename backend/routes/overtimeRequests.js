...
// DELETE /api/overtime-requests/:id - Nhân viên xóa đơn tăng ca khi chưa duyệt, HR xóa đơn đã từ chối
router.delete('/:id', async (req, res) => {
    try {
        ...
        const requestResult = await pool.query(
            `SELECT id, employee_id, status FROM overtime_requests WHERE id = $1`,
            [requestId]
        );
        ...
        if (isHR) {
            ...
            deleteQuery = `DELETE FROM overtime_requests WHERE id = $1 AND status = $2 RETURNING *`;
            ...
            deleteQuery = `DELETE FROM overtime_requests WHERE id = $1 AND employee_id = $2 AND status = $3 RETURNING *`;
        ...
        res.json({
            success: true,
            message: isHR ? 'Đã xóa đơn tăng ca đã từ chối' : 'Đã xóa đơn tăng ca trước khi duyệt'
        });
    } catch (error) {
